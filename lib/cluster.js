...
Cluster.prototype.enableEventing = function(hosts) {
  if (!Array.isArray(hosts)) {
    hosts = [hosts];
  }
...
Cluster.prototype._getCepManager = function() {
  if (this.cepHosts === null) {
    throw new Error('must specify eventing hosts first');
  }

  if (!this.cepManager) {
    this.cepManager = new CepManager(this.cepHosts);
  }

  return this.cepManager;
};

Cluster.prototype.openFeed = function(name, callback) {
  var cepMgr = this._getCepManager()
  cepMgr.openFeed(name, callback);
};
...
Cluster.prototype.openBucket = function(name, password, callback) {
  ...
  bucketDsnObj.options.client_string =
    ...
    ' (node/' + ver.node + '; v8/' + ver.v8 + '; ssl/' + ver.openssl + ')';
