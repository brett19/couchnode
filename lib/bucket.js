 ...
 *
 * It returns a result object containing information about the ping
 * operations that were performed by the SDK.
 *
 * @typedef {function} Bucket.PingCallback
 *
 * @param {undefined|Error} error
 *  The error for the operation. This can either be an Error object
 *  or a value which evaluates to false (null, undefined, 0 or false).
 ...
 *
 * It returns a result object containing information about the diagnostics
 * operations that were performed by the SDK.
 *
 * @typedef {function} Bucket.DiagnosticsCallback
 *
 * @param {undefined|Error} error
 *  The error for the operation. This can either be an Error object
 *  or a value which evaluates to false (null, undefined, 0 or false).
 ...
 * @committed
 */
function Bucket(cluster, options) {
  ...
  var logFunc = logging._currentFunc();
  if (logFunc) {
    this._cb.setLoggingCallback(function(info) {
      logFunc(
        logging.lcbSeverityToLevel(info.severity),
        info.message, {
          subsystem: info.subsys,
          srcfile: info.srcFile,
          srcline: info.srcLine
        });
    });
  }
...
Bucket.prototype.watch = function(filter) {
  var cepMgr = this.cluster._getCepManager();
  return cepMgr.bucket(this._name).addWatch(filter);
};
...
/**
 * Pings the specified services and returns diagnostics
 * information about the state of those connections.
 *
 * @param {ServiceType[]} services
 * @param {Bucket.PingCallback} callback
 *
 * @since 2.4.4
 * @uncommitted
 */
Bucket.prototype.ping = function(services, callback) {
  var servicesVal = 0;

  for (var i = 0; i < services.length; ++i) {
    switch (services[i]) {
      case consts.ServiceType.KeyValue:
        servicesVal |= CONST.PINGSVC_F_KV;
        break;
      case consts.ServiceType.Views:
        servicesVal |= CONST.PINGSVC_F_VIEWS;
        break;
      case consts.ServiceType.Query:
        servicesVal |= CONST.PINGSVC_F_N1QL;
        break;
      case consts.ServiceType.Search:
        servicesVal |= CONST.PINGSVC_F_FTS;
        break;
      default:
  ...
  if (servicesVal === 0) {
    throw new Error('Must specify at least one service to ping');
  }

  var parseCb = function(err, res) {
    if (err) {
      callback(err, null);
      return;
    }
  ...
  this._maybeInvoke(this._cb.ping, [servicesVal, parseCb]);
};

/**
 *
 * @param {Bucket.DiagnosticsCallback} callback
 *
 * @since 2.4.4
 * @uncommitted
 */
Bucket.prototype.diagnostics = function(callback) {
  var parseCb = function(err, res) {
    if (err) {
      callback(err, null);
      return;
  ...
  this._maybeInvoke(this._cb.diag, [parseCb]);
};
...
function _serviceToLcbHttpType(service) {
  switch (service) {
    case consts.ServiceType.Config:
      return CONST.HTTP_TYPE_MANAGEMENT;
    case consts.ServiceType.Views:
      return CONST.HTTP_TYPE_VIEW;
    case consts.ServiceType.Query:
      return CONST.HTTP_TYPE_N1QL;
    case consts.ServiceType.Search:
      return CONST.HTTP_TYPE_FTS;
    case consts.ServiceType.Analytics:
      return CONST.HTTP_TYPE_CBAS;
  }
  throw new Error('invalid service type specified');
}

function _methodToLcbMethodType(method) {
  switch (method.toLowerCase()) {
    case 'get':
      return CONST.HTTP_METHOD_GET;
    case 'post':
      return CONST.HTTP_METHOD_POST;
    case 'put':
      return CONST.HTTP_METHOD_PUT;
    case 'delete':
      return CONST.HTTP_METHOD_DELETE;
  }
...
Bucket.prototype._httpReq =
  function(reqType, reqMethod, user, pass, path, contentType, body, emitter) {
    this._cb.httpRequest(
      reqType,
      reqMethod,
      user,
      pass,
      ...
      function(errCode, val) {
        if (errCode === -1) {
          ...
          var err = new Error('unknown error: ' + errCode);
          emitter.emit('error', err);
        }
      });
  };

Bucket.prototype._http =
  function(service, method, user, pass, path, contentType, body, callback) {
    var reqType = _serviceToLcbHttpType(service);
    var reqMethod = _methodToLcbMethodType(method);

    var req = new HttpResponse();
    ...
    this._maybeInvoke(this._httpReq.bind(this), [reqType, reqMethod, user, pass,
      path, contentType, body, req, invokeCb
    ]);
...
Bucket.prototype._view = function(viewtype, ddoc, name, q, popts, callback) {
  ...
  this._maybeInvoke(this._viewReq.bind(this), [viewtype, ddoc, name, q, popts,
    req, invokeCb
  ]);
    ...
    function(errCode, val) {
      if (errCode === -1) { // Row
        ...
        if (jsonError && jsonError.errors && jsonError.errors.length > 0) {
          var errFirstErr = jsonError.errors[0];

          err = new Error(errFirstErr.msg);
          err.requestID = jsonError.requestID;
          err.code = errFirstErr.code;
          err.otherErrors = [];

          for (var j = 1; j < jsonError.errors.length; ++j) {
            var errNextErr = jsonError.errors[j];
            var errOtherErr = new Error(errNextErr.msg);
            errOtherErr.code = errNextErr.code;
            err.otherErrors.push(errOtherErr);
          }
        } else {
...
Bucket.prototype._n1ql = function(query, params, callback) {
  ...
  this._maybeInvoke(this._n1qlReq.bind(this), [host, query.toObject(params),
    query.isAdhoc, req, invokeCb
  ]);
...
Bucket.prototype.query = function(query, params, callback) {
  ...
  if (query instanceof ViewQuery) {
    return this._view(
      '_view', query.ddoc, query.name, query.options,
      query.postoptions, callback);
  } else if (query instanceof SpatialQuery) {
    return this._view(
      '_spatial', query.ddoc, query.name, query.options,
      query.postoptions, callback);
...
Bucket.prototype.getAndTouch = function(key, expiry, options, callback) {
  ...
  if (typeof expiry !== 'number' || expiry < 0) {
    throw new TypeError(
      'Second argument needs to be 0 or a positive integer.');
  }
  if (typeof options !== 'object') {
...
Bucket.prototype.getAndLock = function(key, options, callback) {
  ...
  this._maybeInvoke(this._cb.get, [key, options.hashkey, options.lockTime, 1,
    callback
  ]);
};
...
Bucket.prototype.getReplica = function(key, options, callback) {
  ...
  this._maybeInvoke(this._cb.getReplica, [key, options.hashkey, options.index,
    callback
  ]);
};
...
Bucket.prototype.remove = function(key, options, callback) {
  ...
  this._maybeInvoke(this._cb.remove, [key, options.hashkey, options.cas,
...
Bucket.prototype._store = function(key, value, options, callback, opType) {
  ...
  this._maybeInvoke(this._cb.store, [key, options.hashkey, value, options.expiry,
    options.cas, opType,
...
Bucket.prototype.counter = function(key, delta, options, callback) {
  ...
  this._maybeInvoke(this._cb.arithmetic, [key, options.hashkey, options.expiry,
    delta, options.initial,
    this._interceptEndure(key, options, 0, callback)
  ]);
};
...
Bucket.prototype.setRemove = function(key, value, options, callback) {
  ...
  this.get(key, options, function(err, res) {
    ...
    self.replace(key, newValues, options, function(err, replaceRes) {
      if (err) {
        if (err.code === errors.keyAlreadyExists && options.cas ===
          undefined) {
...
Bucket.prototype.queuePop = function(key, options, callback) {
  ...
  self.lookupIn(key, options).get('[-1]').execute(function(err, res) {
    ...
    self.mutateIn(key, removeOpts).remove('[-1]').execute(function(err,
      res) {
      if (err) {
        if (err.code === errors.keyAlreadyExists && options.cas ===
          undefined) {
...
Bucket.prototype.mutateIn = function(key, options) {
  ...
  return new MutateInBuilder(this, [key, options.hashkey, options.expiry,
    options.cas, flags
  ]);
};
...
/**
 * Returns a reference to the Node.js library itself.  This is primarily
 * used in cases where you have a bucket reference and need to create
 * query objects for it.
 *
 * @member {Object} Bucket#cbLib
 *
 * @since 2.4.3
 * @uncommitted
 */
Object.defineProperty(Bucket.prototype, 'cbLib', {
  get: function() {
